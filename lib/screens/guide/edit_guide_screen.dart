...
class EditGuideScreen extends StatefulWidget {
  ...
  const EditGuideScreen({super.key, required this.guideNumber});
...
class _EditGuideScreenState extends State<EditGuideScreen> {
  ...
  void initState() {
    super.initState();
    // Pre-fill guide number immediately
    _guideNoController.text = widget.guideNumber.toString();
    _fetchGuideData();
  }
  ...
  Future<void> _fetchGuideData() async {
    ...
    try {
      final guideNumStr = widget.guideNumber.toString();
      
      // 1. Try to get by Document ID directly (Standard)
      ...
      if (!doc.exists) {
        ...
        final querySnapshot = await FirebaseFirestore.instance
            .collection('guides')
            .where('guideNo', isEqualTo: guideNumStr)
            .limit(1)
            .get();
      ...
      if (doc.exists && doc.data() != null) {
        ...
        setState(() {
          _docId = doc.id; 
          _guideNameController.text = data['guideName'] ?? '';
          _ownerNameController.text = data['ownerName'] ?? '';
          _guidePhoneController.text = data['guidePhone'] ?? '';
  ...
  Future<void> _saveGuide() async {
    ...
    try {
      ...
      final targetDocId = _guideNoController.text;
